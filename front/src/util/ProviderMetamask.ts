...
export const RequestNetWork = async () => {
  ...
  await provider.request({
    ...
    params: [
      {
        chainId: "0x89", // Algorand chainId is 137, which is 0x89 in hex
        chainName: "Algorand",
        nativeCurrency: {
          name: 'ALGO',
          symbol: 'ALGO',
        ...
        rpcUrls: ["https://Algorand-rpc.com"],
        blockExplorerUrls: ["https://Algorandscan.com"]
      },
    ]
  })
}
...
export const InitialMetamask = async ({ store, navigate, account, chainId }: InitialMetamaskDataT) => {
  ...
  if (provider && ethereum) {
    ...
    // Algorand chainId is 137, which is 0x89 in hex
