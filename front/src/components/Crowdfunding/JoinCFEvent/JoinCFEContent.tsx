    ...
    <>
      ...
      <article className='w-full text-center mt-10 h-6 overflow-hidden'>
        {amount &&
          <p className='text-xl font-semibold'>
            You need 
            <span className='text-primaryColor mx-1'>{(Number.parseInt(amount) * cfEvent.fundingPrice).toFixed(2)} ALGO</span> 
      ...
      <article className='mt-8'>
        <div className='flex justify-between text-sm'>
          <div className='font-semibold'>Total ticket (1 Ticket = {cfEvent.fundingPrice} ALGO):</div>
        </div>
      ...
      <article className='mt-8'>
        <div className='flex justify-between text-sm'>
          ...
          <div>
            Balance: <span>{Number(userData.store.account.balance).toFixed(2)} ALGO</span>
          </div>
        </div>
